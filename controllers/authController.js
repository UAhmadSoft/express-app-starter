...
exports.signup = catchAsync(async (req, res, next) => {
   ...
   if (process.env.NODE_ENV === 'development')
      activationURL = `${req.headers.origin}/api/v1/confirmMail/${activationToken}`;
   else
      activationURL = `${req.headers.origin}/confirmMail/${activationToken}`;
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
   ...
   if (!email)
      return next(
         new AppError(`Plz provide Email with request`, 400)
      );
   ...
   if (!user)
      return next(
         new AppError(
            `No User Found against this Email : ${email}`,
            400
         )
      );
